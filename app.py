...
st.sidebar.markdown(
    ...
    2. **웹툰화**: '**웹툰화 해줘**'라고 입력하면 업로드된 이미지를 웹툰화 해줍니다.
    3. **이미지 분석**: '**이미지 분석해줘**'라고 입력하면 업로드된 이미지를 분석합니다.
    4. **음악 생성**: 이미지를 업로드하고 '**음악 만들어줘**'를 언급하면 얼굴의 감정을 예측하여 음악을 생성합니다.
    5. **패션 추천 이미지 생성**: 이미지를 업로드하고 '**패션 추천**'을 언급하면 Gemini를 통해 개인의 이미지를 분석하여 패션 추천 이미지를 생성합니다.
    """
)

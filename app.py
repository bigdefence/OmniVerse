...
def main():
    ...
    with st.sidebar:
        ...
        if suno_tab == "Suno Cookie 입력":
            ...
            ### Suno API Key를 얻는 방법
            """)
            st.markdown("<p class='sidebar-text'>[Cookie 얻는 방법](https://github.com/bigdefence/Music-Face)</p>",unsafe_allow_html=True)
            st.markdown("<p class='sidebar-text'>[Suno 웹사이트로 이동하기](https://suno.com/)</p>",unsafe_allow_html=True)

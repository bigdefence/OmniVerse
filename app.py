...
st.sidebar.title("🤗 이미지 업로드와 다양한 기능을 즐기세요!")
st.sidebar.markdown(
    """
    🎨 외모분석
    이미지를 업로드하고 "외모분석"을 외쳐보세요! AI가 이미지 속 외모를 분석해, 인사이트를 제공합니다. 나도 몰랐던 매력을 발견할 수 있을지도?!
    
    🖼️ 웹툰화
    이미지를 웹툰처럼 만들고 싶다면? "웹툰화 해줘"라고 말해보세요. 여러분의 사진이 마치 만화 속 주인공처럼 변신합니다! 나만의 웹툰 주인공을 만들어보세요.
    
    🔍 이미지 분석
    이미지 속 세부적인 정보를 알고 싶다면, "이미지 분석해줘"라고 입력하세요. 이미지 속 숨겨진 이야기를 찾아드릴게요. 누가, 언제, 어디서 찍었는지 궁금하지 않으세요?
    
    🎵 음악 생성
    사진 속 감정을 담은 음악을 듣고 싶다면? 이미지를 업로드하고 "음악 만들어줘"라고 해보세요. AI가 사진 속 감정을 읽고, 그에 맞는 음악을 만들어줍니다. 감정이 담긴 나만의 사운드트랙을 들어보세요.
    
    👗 패션 추천 이미지 생성
    나에게 어울리는 패션을 알고 싶나요? 이미지를 업로드하고 "패션 추천"을 외쳐보세요. AI가 여러분의 이미지를 분석해, 어울리는 패션 스타일을 추천해 드립니다. 새로운 스타일을 시도할 때 딱이죠!
    
    이제 이미지를 업로드하고, 다양한 기능을 마음껏 즐겨보세요! 손쉬운 사용으로 여러분의 일상에 색다른 재미를 더해드립니다.
    """
)
...
def fashion(image, gemini_model,huggingface_api):
    ...
    try:
        response = requests.post(API_URL, headers=headers, json={"inputs": gemini_description})

...
st.sidebar.subtitle("🤗 이미지 업로드와 다양한 기능을 즐기세요!")
st.sidebar.markdown(
    """
    1. 외모분석: 이미지를 업로드하고 "외모 분석해줘"를 입력해보세요. AI가 외모를 분석해 새로운 매력을 찾아드립니다.

    2. 웹툰화: "웹툰화 해줘"라고 입력하면, 사진이 웹툰 주인공처럼 변신합니다.
    
    3. 이미지 분석: "이미지 분석해줘"를 입력해 사진 속 숨겨진 정보를 확인해보세요.
    
    4. 음악 생성: 나만의 음악이 필요하다면, 이미지를 올리고 "음악 만들어줘"라고 해보세요.
    
    5. 패션 추천: 나에게 어울리는 스타일이 궁금하다면, 이미지를 올리고 "패션 추천해줘"를 입력해보세요.
    
    손쉬운 이미지 업로드로 다양한 기능을 즐겨보세요!
    """
)
